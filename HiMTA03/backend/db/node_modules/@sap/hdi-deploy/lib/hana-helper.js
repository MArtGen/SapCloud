...
function printMessagesToConsole(logObj, messages) {
  messages.forEach(function(message) {
    ...
    if (showDetails) {
      ...
      if (id) {
        logObj.logVerbose('%s[%s] %s: %s: %s [%s]', indent, message.TIMESTAMP_UTC, severity, id, message.MESSAGE, message.MESSAGE_CODE);
      } else {
        logObj.logVerbose('%s[%s] %s: %s [%s]', indent, message.TIMESTAMP_UTC, severity, message.MESSAGE, message.MESSAGE_CODE);
      }
      ...
      logObj.logVerbose('%s[%s] %s', indent, message.TIMESTAMP_UTC, message.MESSAGE);
    }
  });
}
...
module.exports = function(host, port, user, password, certificate, dbhosts) {
  ...
  return {
    ...
    createUser: function(username, pw) {
      return function(cb) {
        async.series([
          execute(`CREATE RESTRICTED USER ${  username  } PASSWORD ${  pw}`),
