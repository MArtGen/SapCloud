...
exports.get = function(options, targetCreds, callback){
  const hdiCreds = {
    user: targetCreds.hdi_user ? targetCreds.hdi_user : targetCreds.user,
    password: targetCreds.hdi_user ? targetCreds.hdi_password : targetCreds.password,
  };

  if (Array.isArray(targetCreds.db_hosts)) {
    hdiCreds.hosts = targetCreds.db_hosts;
  } else {
    hdiCreds.host = targetCreds.host;
    hdiCreds.port = targetCreds.port;
  }

  if (targetCreds.certificate) {
    hdiCreds.ca = Array.isArray(targetCreds.certificate) ? targetCreds.certificate : [targetCreds.certificate];
