...
function resolve( model ) {
  ...
  function main() {
    ...
    // - for localized entities
    forEachDefinition( model, createLocalizedConvenienceView );
    // - for localized views
    model._entities.forEach ( createLocalizedConvenienceViewForViews );
    // Phase 6: rewrite associations, exclude associations:
  ...
  function createLocalizedConvenienceView(art) {
    let viewName = 'localized.' + art.name.absolute;
    // if the entity has localized elements
    if (art.$localizedElements && art.$localizedElements.length) {
      createLocalizedDataView( art, viewName, art.$localizedElements );
    }
  }
  ...
  function createLocalizedDataView( base, absolute, textElems ) {
    ...
    populateView( art )
    propagateKeyProps ( art );
    resolveRefs( art );
  }

  function createLocalizedConvenienceViewForViews( art ) {
    if (!options.oldLocalizedConv && art.$localizedElements == undefined && !art.name.absolute.startsWith('localized.') && ['view', 'entity'].includes(art.kind) && art.elements && (!art.abstract || !art.abstract.val)) {
      let viewName = 'localized.' + art.name.absolute;
      ...
      let isLocalized = Object.keys(art.elements).some((elemName) => {
        let elem = art.elements[elemName];
        return !(elem instanceof Array) && hasTruthyProp( elem, 'localized' );
      });

      if (!isLocalized) {
        return; // no localized convenience view needed
      }
      ...
      // - rewrite froms (from entities / views to localized versions if existent)
      const success = rewriteFrom(query, location);
      if (!success) {
        message( null, art.name.location, art, null, 'Info', 'Convenience view cannot be created due to invalid elements in from clause' );
        return; // skip creation
      }
      // - create view
      let localizedView = {
    ...
    // tries to rewrite all 'from' entities to their corresponding localized entities
    // if existent.
    // Returns false in case of invalid / unsupported elments.
    function rewriteFrom(node, location) {
      if (node instanceof Array) {
        return node.every((elem) => rewriteFrom(elem, location));
      }
      if (node.from) {
        return node.from.every((elem) => rewriteFrom(elem, location));
      }
      // rewrite join or union
      if (node.join || node.op) {
        return rewriteFrom(node.args, location);
      }
      if (!node.path) {
        return false; // parse error
      }
      let lastElem = node.path[node.path.length - 1]._artifact;
      if (!lastElem || !['entity', 'view'].includes(lastElem.kind)) {
        // the path doesn't point to an entity (could e.g. point to an element)
        return false;
      }
      let localizedName = 'localized.' + lastElem.name.absolute;
      let newTarget = model.definitions[localizedName] || lastElem;
      // rewrite path either to localized entity or to old entity
      // attention: even for non-localized paths it may be needed to rewrite the path due to namespaces / contexts,
      //            so we always need the absolute path
      node.path = [{ id: newTarget.name.absolute, location }];
      // remove _artifact linkage as it may be outdated
      if (node._artifact) {
        delete node._artifact;
      }
      return true;
    }
  }
...
function copyExpr( expr, location, skipUnderscored, rewritePath ) {
  ...
  let isQuery = proto && expr.op && queryOps[ expr.op.val ];
  for (let prop of Object.getOwnPropertyNames( expr )) {
    let pd = Object.getOwnPropertyDescriptor( expr, prop );
    if (!pd.enumerable || pd.value == null) {// should include all properties starting with _
      if (!skipUnderscored)
        Object.defineProperty( r, prop, pd );
    }
    else if (!proto)
      r[prop] = copyExpr( pd.value, location, skipUnderscored, rewritePath );
    else if (isQuery && prop === 'elements')
      continue; // skip elements
    else if (isQuery && prop === 'from')
      // copy element but don't skip properties with underscore
      // (needed for rewriting localized convenience view's 'from' clause)
      r[prop] = copyExpr( pd.value, location, false );
    else if (prop === 'location')
      r[prop] = location || pd.value;
    else if (rewritePath && prop === 'path' && expr._artifact && !expr._artifact._main)
      r[prop] = [{ id: expr._artifact.name.absolute, location }];
